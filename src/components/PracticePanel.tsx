...
import { 
  Filter, HelpCircle, ArrowRight, CheckCircle2, AlertCircle, Sparkles, Loader2, RefreshCw 
} from "lucide-react";
...
export default function PracticePanel({
  ...
  const handleSelectOption = (optionKey: "A" | "B" | "C" | "D") => {
    ...
    if (correct) {
      ...
      if (attemptsOnQuestion === 0 && newIncorrectQuestionIds.includes(activeQuestion.id)) {
        newIncorrectQuestionIds = newIncorrectQuestionIds.filter(id => id !== activeQuestion.id);
      }
    } else {
      setIsCorrect(false);
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAiExplanation(`⚠️ Không thể kết nối với trí tuệ nhân tạo. Lỗi: ${data.error || "Không rõ"}`);
      }
    } catch (err: any) {
      setAiExplanation(`⚠️ Gặp lỗi đường truyền mạng: ${err.message}`);
    } finally {
      setAiLoading(false);
    }
  };
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 space-y-5">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center justify-between">
          ...
          <div className="text-xs text-slate-500 font-medium">
            Danh sách: <span className="font-bold text-sky-600">{filteredQuestions.length}</span> câu hỏi được lọc
          </div>
        </div>
        ...
        {activeQuestion ? (
          <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="text-base font-bold text-slate-800 leading-normal flex gap-2">
                <span className="text-sky-600 font-mono text-lg font-bold">Q.</span>
            ...
            <div className="grid grid-cols-1 gap-3 pt-2">
              {activeQuestion.options.map((option) => {
                ...
                let textBadgeStyle = "bg-slate-100 text-slate-500 group-hover:bg-slate-200";

                if (isCorrect && isCorrectOption) {
                  // If answered correctly, highlight correct option
                  optionStyle = "bg-emerald-50 border-emerald-300 text-emerald-900 shadow-sm";
                  textBadgeStyle = "bg-emerald-500 text-white";
                } else if (isWrongSelected) {
                  // If wrong and selected previously, highlight as red
                  optionStyle = "bg-rose-50/70 border-rose-200 text-rose-900 line-through opacity-80";
                  textBadgeStyle = "bg-rose-500 text-white";
                } else if (isSelected) {
                  // Just selected but evaluation hasn't finalized or is evaluated
                  optionStyle = "border-sky-500 bg-sky-50/20 text-sky-900 ring-1 ring-sky-500";
                ...
                return (
                  <button
                    key={option.key}
                    disabled={isCorrect !== null && isCorrect && isCorrectOption} // Lock selection once correct
                  ...
                  >
                    <span className={`w-8 h-8 rounded-lg flex items-center justify-center font-mono font-bold text-sm shrink-0 transition ${textBadgeStyle}`}>
                      {option.key}
                    </span>
            ...
            {isCorrect === false && (
              <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 text-xs text-rose-800 flex items-start gap-2.5 animate-pulse">
                ...
                <div>
                  <strong>Lựa chọn chưa chính xác!</strong> Trong hệ thống này, bạn cần suy nghĩ và tìm câu trả lời đúng của câu này để nhớ kiến thức sâu nhất. Hãy đọc kĩ lại code, gợi ý hoặc bấm <strong>Hỏi Trợ Lý AI</strong> bên phải để được hướng dẫn!
                </div>
              </div>
            )}
            ...
            {isCorrect === true && (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-xs text-emerald-800 flex items-start gap-2.5">
                ...
                <div>
                  <strong>Tuyệt vời, câu trả lời chính xác!</strong> Hãy xem phần giải thích chi tiết trong mục hướng dẫn ôn luyện bên phải hoặc chuyển tiếp đến các cột mốc câu hỏi tiếp theo dưới đây.
                </div>
              </div>
            )}
          ...
          <div className="bg-white p-12 rounded-2xl border border-slate-100 shadow-sm text-center">
            ...
            <h3 className="text-base font-bold text-slate-700">Không tìm thấy câu hỏi thích hợp</h3>
            <p className="text-xs text-slate-400 mt-2">Hãy thử đổi bộ lọc chủ đề hoặc độ khó khác trong bộ lọc phía trên.</p>
            <button
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-fit space-y-6">
        <div>
          ...
          <p className="text-slate-400 text-[11px] mt-1">Cách định nghĩa câu trả lời khoa học của Đại học.</p>
        </div>
        ...
        {activeQuestion && (
          <div className="space-y-4">
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/60">
              <div className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">💡 Gợi ý cốt lõi</div>
              <p className="text-slate-600 text-xs leading-relaxed">
                {isCorrect ? activeQuestion.explanation : "Hãy thử chọn một phương án! Hệ thống sẽ kích hoạt giải thích chi tiết ngay tại đây khi bạn bấm đúng để khắc sâu trí nhớ."}
              </p>
            </div>

            <div className="text-[11px] text-slate-400 leading-normal">
              📌 <strong>Mẹo ôn tập:</strong> Trong lập trình hướng đối tượng C++, việc sai liên tục là bình thường. Hãy dùng chức năng <strong>Hỏi Trợ Lý AI</strong> để phân tích từng dòng lệnh của đoạn mã code nếu bạn cảm thấy lạ lẫm.
            </div>
          </div>
        )}
